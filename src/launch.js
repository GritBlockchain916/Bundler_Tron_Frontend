...
import { FULL_NODE, NILE_FULL_NODE, NILE_SUNSWAPV2_ROUTER_ADDRESS, SUNSWAPV2_ROUTER_ADDRESS, WTRX_ADDRESS, NILE_WTRX_ADDRESS, WTRX_DECIMALS, TOKEN_DECIMALS, bundler_url } from './tronConst.js';
...
async function addLiquidity(tokenAddress, routerAddress, tokenAmount, TRXAmount, privateKey) {
    ...
    const optionsAddLiquidity = {
        ...
        callValue: TRXAmount, // Example call value, in this case, 0 TRX
        tokenValue: 0 // No token being transferred
    };
...
async function buyToken(tokenAddress, routerAddress, WTRXAddress, privateKey) {
    const pubKey = tronWeb.address.fromPrivateKey(privateKey);
    const trxAmounts = await tronWeb.trx.getUnconfirmedBalance(pubKey)
    const sellTrx = trxAmounts - tronWeb.toSun(100);
    console.log('sellTrx: ', trxAmounts, sellTrx)
    ...
    const parametersSwap = [
        ...
        { type: 'address', value: pubKey }, // buyer address
...
export async function sellToken(tokenAddress, routerAddress, WTRXAddress, privateKey) {
    ...
    const tx = await approve(tokenAddress, routerAddress, tokenAmount, privateKey)
    console.log(tx.raw_data.contract[0].parameter.value.owner_address)
    const result = await axios.post(`${bundler_url}/api/bundle/sendRawBundle`,
        { raw_bundle_tx: [tx], networkMode: networkMode },
        {
            headers: {
                'Access-Control-Allow-Origin': '*', // Often not needed as it's server-specific, but you can specify it
                'Content-Type': 'application/json', // other headers you might need
    ...
    const parametersSwap = [
        ...
        { type: 'address', value: pubKey }, // seller address
        { type: 'uint256', value: DEADLINEBUY }, // deadline
    ];
    ...
    try {
        ...
        try {
            const signedTransactionSwap = await tronWeb.trx.sign(resultSwap.transaction, privateKey);
            const result = await axios.post(`${bundler_url}/api/bundle/sendRawBundle`,
                { raw_bundle_tx: [signedTransactionSwap], networkMode: networkMode },
                {
                    headers: {
                        'Access-Control-Allow-Origin': '*', // Often not needed as it's server-specific, but you can specify it
                        'Content-Type': 'application/json', // other headers you might need
            ...
            console.error('Error sellToken(sign transaction):', error);
            return error;
        }
    } catch (error) {
        console.error('Error buyToken:', error);
...
export async function initialBuy(request) {
    ...
    const liquidityEth = tronWeb.toSun(request[0].liquidityEth)
    ...
    if (trxOwnerAmounts < liquidityEth) {
        console.log("Not enough TRX to initalBuy!")
        return null;
    }

    const trxMaxTotalInitialBuyAmount = trxOwnerAmounts - liquidityEth - zombieWalletSize * 150
    ...
    try {
        ...
        try {
            ...
            bundle_txs.push(txAddLiquidity)
        } catch (error) {
        ...
        const result = await axios.post(`${bundler_url}/api/bundle/sendRawBundle`,
            { raw_bundle_tx: bundle_txs, networkMode: networkMode },
            {
                headers: {
                    'Access-Control-Allow-Origin': '*', // Often not needed as it's server-specific, but you can specify it
                    'Content-Type': 'application/json', // other headers you might need
